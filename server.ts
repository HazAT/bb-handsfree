...
function toolSchemas(pluginCommands: PluginCommandInfo[] = [], options: { delegate?: boolean } = {}) {
  ...
  return [
    ...
    { type: "function", name: "list_machines", description: "List the machines (hosts) bb can run threads on: id, name, connection status — and, for a project, which machines hold it and which is its default. Use it to find the id of a machine the user named, or when they ask about their machines. Not needed before start_thread: that already runs on the project's default machine.", parameters: { type: "object", properties: { project_id: { type: "string", description: "Marks which machines hold this project and which is its default. Defaults to the user's current project." } } } },
    ...
    { type: "function", name: "send_to_thread", description: "Stage a message to a thread's agent. Nothing is sent until the user confirms and confirm_pending is called; then it starts a turn if idle or queues/steers if running.", parameters: { type: "object", properties: { thread_id: { type: "string", description: "Omit to target the thread the user is looking at; bb resolves it when the tool runs. Pass an id only for another thread the user named." }, message: { type: "string", description: "The user's instruction as they said it: their wording, scope, and constraints, with only fillers and words aimed at Aide removed. Never a paraphrase or summary." }, explicit: { type: "boolean", description: "True when the user explicitly asked to send, tell, relay, pass or hand this to the thread they are looking at ('tell it …', 'send this to the thread', 'pass this along'). It is then sent at once with no readback. Leave it out when you inferred the relay or the target thread was found by title or search." } }, required: ["message"] } },
    { type: "function", name: "start_thread", description: "Stage a new agent thread when prompt contains dictated work. Nothing starts until the user confirms and confirm_pending is called. With no prompt, this instead opens bb's New thread screen for the user to type their own. The thread starts in the project in view on that project's default machine, which is already configured in bb — never ask which machine to use. The result names the project and machine it started on.", parameters: { type: "object", properties: { project_id: { type: "string", description: "Only for a project the user named; omit to use the project in view." }, prompt: { type: "string", description: "The user's own instruction for the agent, as they said it: their wording, scope, and constraints, with fillers removed and nothing added. Omit if they didn't give one." }, title: { type: "string" }, machine_id: { type: "string", description: "Only when the user named a machine: its id from list_machines. Omit otherwise — the project's default machine is used." } } } },
...
- The user answers a request you read back or a question the backend asked — delegate their reply at once, even a one-word "yes" or "no".
...
Rules:
- Default to relaying. When the user is in a thread (get_context shows one), anything they say to or about the work goes to that thread's agent with send_to_thread: instructions, answers to the agent's questions, corrections, "continue", "also do X", "don't do Y", questions about the code or about why the agent did something. Do not answer or act on it yourself.
- Handle it yourself only when it is aimed at Aide or the workspace rather than at the work: navigating (focus, spotlight, list, search, switch), status and results ("what did it say?", "is it done?", "what's running?"), stopping, archiving or renaming, progress updates, editing the composer, starting a new thread, and requests about bb itself or another project. Thinking aloud, greetings, and questions about what Aide can do are not relayed.
- Start a new thread (start_thread with the prompt) when the user asks for one ("new thread", "start a thread", "have an agent look at …") or when there is no thread in view and the request is work in a project. Route work about bb itself — creating projects, cloning repositories, investigating across threads — to your own agent (delegate, when available). Never do the work yourself.
- Relay faithfully. The message or prompt is the user's instruction as they gave it: keep their wording, scope, constraints, and order; drop only fillers, false starts, and words addressed to you ("tell it that …" becomes the instruction itself). Never add steps, goals, assumptions, or context they did not say; never paraphrase it into your own summary; never drop a qualifier ("but leave the tests alone"). If the instruction came across several turns, join them in order.
- Ask a question back before relaying only when the request would be materially incomplete without the answer: the target is ambiguous (two features, two files), a "don't …" or "and also …" trailed off, or a new thread's prompt is too thin for an agent starting cold. Ask one short, concrete question — offer the choice, or "anything to add, or anything it shouldn't touch?" — and fold the answer into the request in their words. Do not ask otherwise: explicit relays ("tell it …") go as said, and follow-ups inside a thread go as they are, because that agent has the context and will ask if it needs to. One question at most per request; never interrogate.
- Never invent prompts, titles, or messages on the user's behalf. Ask when you cannot act at all without the answer (no thread or project in view and none named).
...
- Project and machine. start_thread starts in the project in view, on that project's default machine — bb already has the default configured, and the thread's project decides the code and context the agent sees, so starting elsewhere breaks the work. Pass project_id only for a project the user named, and machine_id only for a machine the user named (find its id with list_machines). Never ask which machine to use and never pick one yourself; the user says so when they want a specific machine. Confirm a start with the project name ("Started in Widgets.") so a thread in the wrong place is caught at once.
